  ...
  return (
    <aside className="w-52 shrink-0 flex flex-col border-r border-hm-border bg-hm-surface h-screen sticky top-0 overflow-y-auto">
      ...
      <nav className="flex-1 py-2">
        <NavItem to="/app" label="Inicio" end onClick={onNavigate} />
        <NavItem to="/app/mi-jornada" label="Mi Jornada" onClick={onNavigate} />

        <GroupLabel>Operaciones</GroupLabel>
        <NavItem to="/app/activo360" label={activoPlural} onClick={onNavigate} />
        {moduleEnabled('taller') && <NavItem to="/app/taller" label="Taller y Servicio" onClick={onNavigate} />}
        {moduleEnabled('inventario') && <NavItem to="/app/repuestos" label="Inventario" onClick={onNavigate} />}
        <NavItem to="/app/postventa" label="Postventa" badge="Base" onClick={onNavigate} />
        {moduleEnabled('alquileres') && permiteAlquileres && <NavItem to="/app/alquileres" label="Rental" onClick={onNavigate} />}

        {canManage && (
          <>
            <GroupLabel>Comercial</GroupLabel>
            {moduleEnabled('crm') && <NavItem to="/app/leads" label="CRM" onClick={onNavigate} />}
            <NavItem to="/app/clientes" label="Clientes" onClick={onNavigate} />
            {moduleEnabled('crm') && <NavItem to="/app/cotizaciones" label="Cotizaciones" onClick={onNavigate} />}
            {moduleEnabled('ventas') && <NavItem to="/app/ventas" label="Ventas" onClick={onNavigate} />}
          </>
        )}
        ...
        {canManage && (
          <>
            ...
            <NavItem to="/app/facturacion" label="Documentos y Cobranzas" onClick={onNavigate} />
            {moduleEnabled('tesoreria') && <NavItem to="/app/tesoreria" label="Tesorería" onClick={onNavigate} />}
            {moduleEnabled('tesoreria') && <NavItem to="/app/proveedores" label="Proveedores" onClick={onNavigate} />}
            <NavItem to="/app/aprobaciones" label="Aprobaciones" onClick={onNavigate} />
          </>
        )}
        ...
        {isOwner && (
          <>
            ...
            <NavItem to="/app/ceo" label="Gerencia" onClick={onNavigate} />
            <NavItem to="/app/alertas" label="Alertas" onClick={onNavigate} />
          </>
        )}
        ...
        {isOwner && (
          <>
            ...
            <NavItem to="/app/usuarios" label="Usuarios y accesos" onClick={onNavigate} />
            <NavItem to="/app/configuracion" label="Configuración" onClick={onNavigate} />
            <NavItem to="/app/integraciones" label="Integraciones" onClick={onNavigate} />
          </>
        )}
      </nav>
