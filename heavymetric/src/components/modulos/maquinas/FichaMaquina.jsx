...
export default function FichaMaquina({ isOpen, onClose, maquinaId }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Ficha Médica y Rentabilidad" maxWidth="max-w-5xl">
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
            ...
            <section>
              ...
              <div className="max-h-[300px] overflow-y-auto pr-2 flex flex-col gap-3">
                {ots.length === 0 ? (
                  ...
                  ots.map(ot => (
                    <div key={ot.id} className="bg-hm-surface2/20 p-4 rounded-lg border border-hm-border/50 hover:border-hm-accent/30 transition-colors">
          ...
          <div className="flex justify-end pt-4 border-t border-hm-border">
            <button 
              onClick={onClose}
              className="px-6 py-2 bg-hm-surface2 hover:bg-hm-surface border border-hm-border rounded font-mono text-xs text-white transition-colors"
            >
              CERRAR FICHA
            </button>
          </div>
