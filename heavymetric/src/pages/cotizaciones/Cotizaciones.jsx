...
function ModalCotizacion({ isOpen, onClose, cotizacion, clientes, leads, onConfirm }) {
  ...
  const [form, setForm] = useState({
    cliente_id: '', lead_id: '', fecha_vencimiento: '', condiciones: '', notas: '',
  })
  ...
  useEffect(() => {
    if (cotizacion) {
      ...
      setItems(cotizacion.items?.length > 0
        ? cotizacion.items.map(i => ({ tipo_item: i.tipo_item, descripcion: i.descripcion, cantidad: i.cantidad, precio_usd: i.precio_usd, tasa_iva: i.tasa_iva }))
        : [{ ...EMPTY_ITEM }]
      )
    } else {
      setForm({ cliente_id: '', lead_id: '', fecha_vencimiento: '', condiciones: '', notas: '' })
      setItems([{ ...EMPTY_ITEM }])
    }
  }, [cotizacion, isOpen])
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.cliente_id && !form.lead_id) { toast.error('Seleccioná un cliente o un lead'); return }
    const hasEmptyDesc = items.some(i => !i.descripcion.trim())
    if (hasEmptyDesc) { toast.error('Completá la descripción de todos los ítems'); return }
    setSaving(true)
    try {
      await onConfirm({ ...form, created_by: perfil?.id, organization_id: perfil?.organization_id }, items)
    } finally { setSaving(false) }
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={cotizacion ? `Editar Cotización #${cotizacion.numero_cotizacion}` : 'Nueva Cotización'} maxWidth="max-w-3xl">
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={form.lead_id} onChange={e => setF('lead_id', e.target.value)} className="select-hm">
              <option value="">— Ninguno —</option>
              {leads.filter(l => !['Ganado','Perdido'].includes(l.estado)).map(l => (
                <option key={l.id} value={l.id}>{l.empresa || l.nombre} ({l.estado})</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input label="Fecha de vencimiento" type="date" value={form.fecha_vencimiento} onChange={e => setF('fecha_vencimiento', e.target.value)} />
          <Input label="Condiciones comerciales" value={form.condiciones} onChange={e => setF('condiciones', e.target.value)} placeholder="Ej: 30 días, sin interés..." />
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addItem} className="text-[10px] font-mono font-bold text-hm-accent hover:underline">
              + AGREGAR ÍTEM
            </button>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-[120px_1fr_70px_90px_80px_28px] gap-2 px-2">
              {['TIPO','DESCRIPCIÓN','CANT.','PRECIO USD','IVA',''].map((h,i) => (
                <span key={i} className="text-[9px] font-mono text-hm-muted uppercase tracking-widest">{h}</span>
              ))}
            </div>

            {items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-[120px_1fr_70px_90px_80px_28px] gap-2 items-center bg-hm-surface2/30 rounded-lg p-2">
                <select value={item.tipo_item} onChange={e => setItem(idx, 'tipo_item', e.target.value)} className="select-hm text-xs py-1.5">
                  {TIPOS.map(t => <option key={t}>{t}</option>)}
                </select>
                <input
                  value={item.descripcion} onChange={e => setItem(idx, 'descripcion', e.target.value)}
                ...
                />
                <input
                  type="number" min="0" step="0.01" value={item.precio_usd}
                ...
                />
                <select value={item.tasa_iva} onChange={e => setItem(idx, 'tasa_iva', e.target.value)} className="select-hm text-xs py-1.5">
                  <option value={0}>0%</option>
                  <option value={0.105}>10.5%</option>
                  <option value={0.21}>21%</option>
                  <option value={0.27}>27%</option>
                </select>
                <button type="button" onClick={() => removeItem(idx)} disabled={items.length === 1} className="text-red-400/60 hover:text-red-400 disabled:opacity-20 text-base leading-none">×</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-hm-surface2/40 rounded-lg p-4 border border-hm-border flex flex-col gap-1.5 self-end w-full max-w-xs ml-auto">
          ...
          <div className="flex justify-between text-sm text-hm-muted font-mono">
            <span>IVA</span><span>{formatUSD(iva)}</span>
          </div>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-end bg-hm-surface2/20 p-4 rounded-lg border border-hm-border/50">
        <div className="flex-1">
          ...
          <Input placeholder="Cliente, lead, número..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setFiltroEstado('todos')} className={`px-3 py-2 rounded text-[10px] font-bold tracking-widest border transition-all ${filtroEstado === 'todos' ? 'bg-hm-accent border-hm-accent text-white' : 'bg-hm-surface border-hm-border text-hm-muted hover:border-hm-accent/50'}`}>
            TODOS
          </button>
          {ESTADOS.map(e => (
            <button key={e} onClick={() => setFiltroEstado(filtroEstado === e ? 'todos' : e)} className={`px-3 py-2 rounded text-[10px] font-bold tracking-widest border transition-all ${filtroEstado === e ? 'bg-hm-accent border-hm-accent text-white' : 'bg-hm-surface border-hm-border text-hm-muted hover:border-hm-accent/50'}`}>
              {e.toUpperCase()}
            </button>
          ))}
        </div>
              ...
              paginadas.map(cot => {
                ...
                return (
                  <tr key={cot.id} className="border-b border-hm-border hover:bg-hm-surface2/30 transition-colors group">
                    ...
                    <td className="p-4"><Badge variant={ESTADO_VARIANT[cot.estado]}>{cot.estado}</Badge></td>
                    <td className="p-4 font-mono text-xs">
                      {vence
                        ? <span className={vencida ? 'text-red-400' : 'text-hm-muted'}>{vence.toLocaleDateString('es-AR')}</span>
