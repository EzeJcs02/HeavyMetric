  ...
  const vencimientos = useMemo(() => {
    const realesCobros = transacciones
      ...
      .map((t) => ({
        id: `real-cobro-${t.id}`,
        tipo: 'cobro',
        tercero: t.cliente?.razon_social || t.cliente?.nombre_comercial || 'Cliente',
        concepto: t.concepto || 'Cobranza pendiente',
        cuota: '-',
        importe: t.monto_total_ars || (t.monto_total_usd || 0) * (t.tipo_cambio_bna || finanzas.tipoCambio?.venta || 1),
    ...
    const realesPagos = compras
      ...
      .map((c) => ({
        id: `real-pago-${c.id}`,
        tipo: 'pago',
        tercero: c.proveedor?.empresa || 'Proveedor',
        concepto: c.concepto || 'Pago pendiente',
        cuota: '-',
        importe: c.total_ars || (c.total_usd || 0) * (finanzas.tipoCambio?.venta || 1),
