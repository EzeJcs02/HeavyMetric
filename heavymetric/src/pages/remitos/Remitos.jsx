...
function ModalRemito({ isOpen, onClose, clientes, onConfirm }) {
  const [form, setForm] = useState({
    tipo: 'salida', cliente_id: '', fecha: new Date().toISOString().slice(0,10),
  ...
  useEffect(() => {
    if (isOpen) {
      setForm({ tipo: 'salida', cliente_id: '', fecha: new Date().toISOString().slice(0,10), destino: '', observaciones: '' })
      setItems([{ ...EMPTY_ITEM }])
    }
  }, [isOpen])
  ...
  const handleSubmit = async (e) => {
    ...
    if (validItems.length === 0) { toast.error('Agregá al menos un ítem con descripción'); return }
    setSaving(true)
    try {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Nuevo Remito" maxWidth="max-w-2xl">
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Fecha *" type="date" value={form.fecha} onChange={e => setF('fecha', e.target.value)} required />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Destino / Referencia" value={form.destino} onChange={e => setF('destino', e.target.value)} placeholder="Destino, obra, sector..." />
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-[90px_1fr_80px_80px_28px] gap-2 items-center bg-hm-surface2/30 rounded-lg p-2">
                <input
                  value={item.codigo} onChange={e => setItem(idx, 'codigo', e.target.value)}
                  placeholder="SKU..."
                ...
                />
                <select value={item.unidad} onChange={e => setItem(idx, 'unidad', e.target.value)} className="select-hm text-xs py-1.5">
                  {UNIDADES.map(u => <option key={u}>{u}</option>)}
                </select>
                <button type="button" onClick={() => removeItem(idx)} disabled={items.length === 1}
                  className="text-red-400/60 hover:text-red-400 disabled:opacity-20 text-base leading-none">×</button>
              </div>
            ))}
          </div>
        </div>

        <Input label="Observaciones" value={form.observaciones} onChange={e => setF('observaciones', e.target.value)} placeholder="Notas adicionales..." />
...
export default function Remitos() {
  ...
  useEffect(() => { setPage(1) }, [filtroEstado, filtroTipo, busqueda])
      ...
      <div className="flex flex-col md:flex-row gap-4 items-end bg-hm-surface2/20 p-4 rounded-lg border border-hm-border/50">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setFiltroEstado('todos')}
            className={`px-3 py-2 rounded text-[10px] font-bold tracking-widest border transition-all ${filtroEstado === 'todos' ? 'bg-hm-accent border-hm-accent text-white' : 'bg-hm-surface border-hm-border text-hm-muted hover:border-hm-accent/50'}`}>
            TODOS
          </button>
          {ESTADOS.map(e => (
            <button key={e} onClick={() => setFiltroEstado(filtroEstado === e ? 'todos' : e)}
              className={`px-3 py-2 rounded text-[10px] font-bold tracking-widest border transition-all ${filtroEstado === e ? 'bg-hm-accent border-hm-accent text-white' : 'bg-hm-surface border-hm-border text-hm-muted hover:border-hm-accent/50'}`}>
              {e.toUpperCase()}
            </button>
          ))}
        </div>
        <div className="flex gap-2">
          {['todos', ...TIPOS].map(t => (
            <button key={t} onClick={() => setFiltroTipo(filtroTipo === t ? 'todos' : t)}
              className={`px-3 py-2 rounded text-[10px] font-bold tracking-widest border transition-all ${filtroTipo === t ? 'bg-hm-accent/20 border-hm-accent text-hm-accent' : 'bg-hm-surface border-hm-border text-hm-muted hover:border-hm-accent/50'}`}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={r.id} className="border-b border-hm-border hover:bg-hm-surface2/30 transition-colors group">
                ...
                <td className="p-4"><Badge variant={ESTADO_VARIANT[r.estado]}>{r.estado}</Badge></td>
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => generateRemitoPDF({
                      ...r,
                      numero_comprobante: r.numero_remito,
                      tipo: TIPO_LABEL[r.tipo] || r.tipo,
                    })}
                      className="px-2 py-1 text-[10px] font-mono font-bold border border-hm-border rounded hover:border-blue-400 hover:text-blue-400 transition-colors">
                      PDF
                    </button>
                    {canEdit && r.estado === 'borrador' && (
                      <button onClick={() => setCambiandoEstado({ remito: r, estado: 'emitido' })}
                    ...
                    {canEdit && r.estado === 'emitido' && (
                      <button onClick={() => setCambiandoEstado({ remito: r, estado: 'entregado' })}
