...
export default function Alquileres() {
  ...
  const handleGenerarAlquilerPDF = async (contrato) => {
    setGenerandoPdf(contrato.id)

    try {
      const { generateAlquilerPDF } = await import('../../lib/pdfGenerator')
      generateAlquilerPDF(contrato)
    } catch (err) {
  ...
  const maquinasGrid = useMemo(() => {
    return maquinas.filter((m) => {
      ...
      const matchSearch =
        m.nombre_unidad.toLowerCase().includes(query) ||
  ...
  const contratosFiltrados = useMemo(() => {
    return contratos.filter((c) => {
      ...
      return (
        ...
        c.nombre_unidad.toLowerCase().includes(query) ||
    ...
    <div className="flex flex-col gap-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-hm-muted text-sm mt-1">
            Control de contratos, vencimientos y disponibilidad de flota.
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-end bg-hm-surface/50 backdrop-blur-sm p-5 rounded-xl border border-hm-border/50 shadow-premium">
        <div className="flex-1 w-full">
          <label className="text-[10px] font-bold text-hm-muted mb-1.5 block uppercase tracking-[0.15em]">
            Buscar en Flota o Contratos
          </label>
        ...
        <div className="flex gap-2 p-1 bg-hm-bg/50 rounded-lg border border-hm-border/30">
          {[
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-[10px] font-bold tracking-wider transition-all duration-200 ${
                filterStatus === f.id
      ...
      <section>
        <h2 className="font-mono text-sm text-hm-muted mb-4 tracking-wider uppercase">
          Estado de Flota
        </h2>
        ...
        {loadingMaq ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-32 bg-hm-surface2 rounded animate-pulse"></div>
            ))}
          </div>
            ...
            {maquinasGrid.map((m) => {
              ...
              return (
                <Card key={m.id} className={`p-4 border ${bgClass} transition-colors hover:bg-hm-surface2/50`}>
                  ...
                  <div className="text-xs font-mono text-hm-muted mt-3">
                    PATENTE: {m.patente}
                  </div>
                </Card>
              )
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-mono text-sm text-hm-muted tracking-wider uppercase">
            Contratos Activos
          </h2>
          ...
          <div className="flex items-center gap-1 bg-hm-surface2 border border-hm-border rounded-lg p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-semibold transition-colors ${
                !vistaCalendario ? 'bg-hm-accent text-hm-bg' : 'text-hm-muted hover:text-hm-text'
              }`}
            >
              Lista
            </button>
                  ...
                  contratosPaginados.map((c) => (
                    <tr key={c.id} className="border-b border-hm-border hover:bg-hm-surface2/30 transition-colors">
                      ...
                      <td className="p-4">
                        <Badge
                          variant={
                            c.estado_vencimiento === 'vencido'
                              ? 'taller'
                              : c.estado_vencimiento === 'por_vencer'
                                ? 'ventas'
                                : 'info'
                          }
                        >
                      ...
                      <td className="p-4 text-right flex gap-2 justify-end">
                        <button
                          onClick={() => handleGenerarAlquilerPDF(c)}
