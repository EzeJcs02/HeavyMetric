...
export function useReportes(desde = null, hasta = null) {
  ...
  const fetchReportes = async () => {
    ...
    try {
      ...
      setRentabilidad(
        Object.values(porCliente).sort((a, b) => b.total_facturado - a.total_facturado)
      )
      ...
      setUtilizacion(
        (maqRes.data || [])
          .map(m => ({ ...m, dias_alquilados: diasPorMaquina[m.id] || 0 }))
          .sort((a, b) => b.dias_alquilados - a.dias_alquilados)
      )

      setServicePendiente(
        (maqRes.data || [])
          .filter(m => ['urgente', 'vencido', 'proximo'].includes(m.estado_service))
